...
class AdvancedContentReader {
    ...
    async readAndAnswer(query) {
        try {
            ...
            // HANYA handle garansi queries yang sangat spesifik untuk avoid override FAQ/SOP
            if (!analysis.isSpecific || !analysis.infoType || analysis.infoType !== 'garansi') {
                return null; // Not a specific garansi question
            }
    ...
    detectSpecificInfoType(query) {
        // Garansi questions - PRIORITY 1 (Natural response)
        if (/\b(garansi|warranty)\b.*\b(berapa|lama|hari|bulan)\b/.test(query) || 
            /\b(berapa|lama)\b.*\b(garansi|warranty)\b/.test(query)) {
            return 'garansi';
        }
        
        // DON'T handle harga queries here - let them go to product template
        // This ensures full product template is shown first
