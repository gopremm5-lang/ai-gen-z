...
async function handleUserMessage(msg, sender) {
  try {
    ...
    // 0.5. CHECK CONVERSATION CONTEXT - Smart contextual responses
    try {
      const contextualResponse = await conversationContext.getContextualResponse(sender, trimmedMsg);
      if (contextualResponse) {
        console.log(`🧠 Contextual response generated for returning customer`);
        // Still track this interaction but return contextual response
        await conversationContext.trackInteraction(sender, trimmedMsg, contextualResponse, { source: 'contextual' });
        return contextualResponse;
      }
    } catch (contextError) {
      console.warn('Error in conversation context:', contextError.message);
      // Continue with normal flow
    }

    // 0. ADVANCED CONTENT READING - HIGHEST PRIORITY for specific info queries
    try {
    ...
    if (lowerMsg.includes('produk apa aja') || lowerMsg.includes('ada produk apa') || lowerMsg.includes('list produk') || lowerMsg === 'menu') {
      return textManager.getText('product_info', 'product_catalog');
    }
    ...
    if (lowerMsg.includes('harga semua') || lowerMsg.includes('berapa harga semua') || (lowerMsg.includes('harga') && lowerMsg.includes('produk') && !existingProducts.some(p => lowerMsg.includes(p)))) {
      return textManager.getText('product_info', 'general_pricing');
    }

    // Handle missing products with natural responses
    if (lowerMsg.includes('spotify')) {
      return textManager.getText('product_info', 'spotify_unavailable');
    }

    if (lowerMsg.includes('canva')) {
      return textManager.getText('product_info', 'canva_unavailable');
    }

    // Handle products yang ada tapi mungkin tidak ke-detect di fuzzy search
    for (const product of existingProducts) {
      if (lowerMsg.includes(product) && (lowerMsg.includes('harga') || lowerMsg.includes('berapa'))) {
        // Force product info with natural follow-up
        try {
          const productInfo = await getProductInfo(product, 'harga');
          if (productInfo && !productInfo.includes('tidak ditemukan')) {
            return productInfo;
          }
        } catch (error) {
    ...
    if (lowerMsg.includes('diskon') || lowerMsg.includes('discount') || (lowerMsg.includes('bisa') && lowerMsg.includes('murah'))) {
      return textManager.getText('mood_responses', 'discount_inquiry');
    }
    ...
    // 1. ADVANCED CONTENT READING - PRIORITY 1 for specific info queries
    try {
      const naturalResponse = await advancedContentReader.readAndAnswer(trimmedMsg);
      if (naturalResponse) {
        console.log(`🧠 Advanced reader generated natural response`);
    ...
    // 2. Mood detector - untuk handling emosi (setelah specific queries)
    try {
    ...
    // 3. FAQ/SOP - prioritas tinggi untuk pertanyaan umum
    try {
    ...
    if (trimmedMsg.length < 3 || !/[a-zA-Z]/.test(trimmedMsg)) {
      return textManager.getText('greeting', 'clarification_request');
    }
    ...
    if (!hasValidWords && trimmedMsg.length < 10) {
      return textManager.getText('error_messages', 'no_understanding');
    }
    ...
    const errorResponse = textManager.getText('error_messages', 'system_error');
    
    // Track error interaction
    try {
      await conversationContext.trackInteraction(sender, trimmedMsg, errorResponse, { source: 'error', error: error.message });
    } catch (trackError) {
      console.warn('Error tracking interaction:', trackError.message);
    }
...
// Wrapper function to ensure all responses are tracked
async function handleUserMessageWithTracking(msg, sender) {
  try {
    const response = await handleUserMessage(msg, sender);
    
    // Track interaction if we got a response
    if (response) {
      try {
        await conversationContext.trackInteraction(sender, msg, response, { source: 'hybrid_handler' });
      } catch (trackError) {
        console.warn('Error tracking interaction in wrapper:', trackError.message);
      }
    }
    
    return response;
  } catch (error) {
    console.error('Error in message handling with tracking:', error);
    return await handleUserMessage(msg, sender); // Fallback to original
  }
}

module.exports = { handleUserMessage, handleUserMessageWithTracking };
