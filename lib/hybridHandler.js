...
// Deteksi intent untuk produk - lebih cerdas dalam mengenali maksud user
function detectProductIntent(msg) {
  const lower = msg.toLowerCase();
  
  // Keywords yang menunjukkan user TIDAK ingin info produk
  const nonProductIntents = [
    // Pertanyaan umum
    'bagaimana', 'gimana', 'cara', 'kenapa', 'mengapa', 'kapan', 'dimana', 'kok',
    // Keluhan/masalah
    'error', 'gagal', 'tidak bisa', 'gak bisa', 'masalah', 'kendala', 'trouble', 'rusak', 'bermasalah',
    // Pertanyaan harga/info umum
    'berapa harga', 'harga berapa', 'berapa biaya', 'biaya berapa', 'harganya', 'biayanya',
    // Pertanyaan garansi/layanan
    'garansi', 'warranty', 'claim', 'refund', 'tukar', 'ganti', 'replace', 'rusak',
    // Pertanyaan pembayaran
    'bayar', 'payment', 'transfer', 'dana', 'ovo', 'gopay', 'qris', 'bank', 'rekening',
    // Pertanyaan proses
    'proses', 'status', 'sudah sampai', 'kapan selesai', 'sudah jadi', 'belum sampai',
    // Pertanyaan login/akses
    'login', 'masuk', 'akses', 'password', 'username', 'email', 'akun saya',
    // Keluhan layanan
    'lambat', 'lama', 'telat', 'pending', 'antri', 'nunggu'
  ];
  
  // Jika mengandung kata-kata non-product intent, jangan return produk
  if (nonProductIntents.some(intent => lower.includes(intent))) {
    return false;
  }
  ...
  // Jika mengandung product intent keywords, lanjut ke fuzzy search
  return productIntents.some(intent => lower.includes(intent));
}
...
async function fuzzySearchProduk(msg) {
  ...
  if (produkNames.length) {
    ...
    if (isValidMatch && bestMatch.rating > 0.5) { // Naikkan threshold untuk akurasi
    ...
    if (isValidMatch && bestMatch.rating > 0.4) {
