/**
 * ATTENDANCE MANAGER
 * Sistem absensi admin via WhatsApp dengan dashboard dan notifikasi owner
 * 
 * Features:
 * - Absen mulai, istirahat, close via chat
 * - Dashboard attendance di web admin
 * - Notifikasi real-time ke owner
 * - Analytics jam kerja dan produktivitas
 */
...
class AttendanceManager {
     ...
     */
    async handleAttendanceCommand(content, sock, sender, remoteJid, message, isAdmin) {
        if (!isAdmin) {
            return null; // Only for admins
        }
    ...
    async clockIn(adminNumber, sock, sender, remoteJid, message) {
        try {
            ...
            // Get admin info
            const adminInfo = await this.getAdminInfo(adminNumber);
    ...
    async getAdminInfo(adminNumber) {
        try {
            const moderators = await loadJson('moderators.json') || [];
            const admin = moderators.find(mod => mod.number === adminNumber);
            
            if (admin) {
                return { name: admin.name, role: 'moderator' };
            }
            
            // Check if owner
            const config = require('../config');
            if (adminNumber === config.owner_number) {
                return { name: config.owner_name || 'Owner', role: 'owner' };
            }
            
            return { name: `Admin ${adminNumber.slice(-4)}`, role: 'admin' };
        } catch (error) {
            return { name: `Admin ${adminNumber.slice(-4)}`, role: 'admin' };
        }
    }
